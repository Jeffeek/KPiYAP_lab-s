...
namespace Lab_no4.Models
{
	internal class Decagon
	{
		...
		public double GetPerimeter()
		{
			...
			for (var i = 0; i < allPoints.Length - 1; i++) P += GetLengthFromPointToPoint(allPoints[i], allPoints[i + 1]);
		...
		{
			var result = Math.Sqrt((first.X - first.X) * (second.X - first.X) + (second.Y - first.Y) * (second.Y - first.Y));
