...
namespace Lab_no18._2
{
    public sealed class ShopMenu
    {
	    ...
	    private void AddPC()
	    {
		    ...
		    if (deXElement != null)
		    {
				...
				deXElement.XPathSelectElement("InStock").Value =
					(int.Parse(deXElement.XPathSelectElement("InStock").Value) + 1).ToString();
				PrintProducts(_document);
				PrintMenu();
				return;
		    }
			...
			var newPc = $"<pc><Name>{name}</Name><characteristics><characteristic>{cpu}</characteristic><characteristic>{gpu}</characteristic></characteristics><InStock>1</InStock></pc>";
	    ...
	    private void RemovePC()
	    {
			...
			pc.Element(XName.Get("InStock")).Value = (int.Parse(pc.Element(XName.Get("InStock")).Value) - 1).ToString();
			if ((int.Parse(pc.Element(XName.Get("InStock")).Value) == 0))
				products.Remove(pc);
		...
		private void PrintMenu()
	    {
			...
			parse:
			var parseResult = int.TryParse(_provider.In(), out var result);
			if (!parseResult || result < 1 || result > _menuChoice.Count) goto parse;
			_menuChoice[result]();
	    }
