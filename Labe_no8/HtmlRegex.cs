...
namespace Labe_no8
{
	public class HtmlRegex
	{
		...
		private string ReadFile()
		{
			...
			string result;
			using (var stream = new StreamReader(Path)) result = stream.ReadToEnd();
		...
		private void CreateHtmlDoc(string textInBody)
		{
			...
			using (var sw = new StreamWriter($"{Directory.GetCurrentDirectory()}\\MyHtml.html")) sw.Write(sb.ToString());
